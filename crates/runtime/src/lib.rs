...
impl Runtime {
    ...
    pub fn set_variable<N: AsRef<str>>(&self, name: N, value: Value) -> Value {
    ...
    fn eval_code_block(&mut self, mut body: Body) -> Value {
        match body.stmts.len() {
            0 => Value::None,
            1 => self.eval_statement(body.stmts.remove(0)),
            _ => {
                let statement = body.stmts.remove(0);
    ...
    pub fn eval_program(&mut self, program: Body) -> Value {
        let value = self.eval_code_block(program);

        self.returned(value)
    }

    pub fn eval_app(&mut self, astRoot: Root) -> Value {
        let program=astRoot.r#mod.top_levels;

        let value = self.eval_app_block(program);

        self.returned(value)
    }
    fn eval_app_block(&mut self, mut stmts: Vec<TopLevel>) -> Value {
        match stmts.len() {
            0 => Value::None,
            1 => self.eval_top_level(stmts.remove(0)),
            _ => {
                let statement = stmts.remove(0);
                let value = self.eval_top_level(statement);
